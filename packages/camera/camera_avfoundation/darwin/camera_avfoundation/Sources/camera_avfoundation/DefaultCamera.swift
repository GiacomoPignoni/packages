...
final class DefaultCamera: NSObject, Camera {
  ...
  private func makePhotoSettings(useShaderPath: Bool, isHEIF: Bool) -> AVCapturePhotoSettings {
        ...
        .contains(kCVPixelFormatType_32BGRA)
    {
      ...
      // `imageQuality` reaches AVFoundation only here, on the compressed-delivery
      // branch. On the BGRA branch above nothing is encoded by AVFoundation — the
      // renderer re-encodes from VRAM — so the setting has nothing to act on.
      if isHEIF {
